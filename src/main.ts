...
(async () => {
	if (transport === 'stdio') {
		...
		if (!accessToken) {
			console.error('google-maps-places-mcp: GOOGLE_ACCESS_TOKEN required for stdio transport');
		...
		console.error('Google Maps Places MCP server running on stdio');
	} else if (transport === 'http') {
		if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
			console.error('google-maps-places-mcp: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET required for HTTP transport');
		...
		const oauthMetadata: OAuthMetadata = {
			...
			scopes_supported: PLACES_SCOPES,
		};
		...
		const protectedResourceMetadata: OAuthProtectedResourceMetadata = {
			...
			scopes_supported: PLACES_SCOPES,
			resource_name: 'Google Maps Places MCP Server',
			resource_documentation: 'https://github.com/domdomegg/google-maps-places-mcp',
		};
		...
		app.post('/register', (req: Request<object, object, OAuthClientMetadata>, res) => {
			const response: OAuthClientInformationFull = {
				...req.body,
				client_id: 'google-maps-places-mcp',
		...
		const httpServer = app.listen(port, () => {
			console.error(`Google Maps Places MCP server running on ${baseUrl}/mcp`);
		});
